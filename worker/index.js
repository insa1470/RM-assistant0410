...
async function handleUpload(request, env) {
  ...
  `).bind(
    ...
    purpose ? purpose.trim() : null, city ? city.trim() : null, branch ? branch.trim() : null,
    is8PlusE ? 1 : 0,
    tmplJson,
    followUp || null,
    hqLeader ? 1 : 0
...
async function handleRecords(request, env) {
  ...
  const recordType = url.searchParams.get('type') || null;
  ...
  if (recordType) { where.push('type = ?'); p.push(recordType); }
...
/* ──────────────────────────────────────────
   POST /api/admin/delete-record — 刪除單筆紀錄（管理員）
   Body: { password, id }
────────────────────────────────────────── */
async function handleDeleteRecord(request, env) {
  if (!checkAdmin(request, env)) return jsonRes({ error: '密碼錯誤' }, 401);
  const { id } = await request.json();
  if (!id) return jsonRes({ error: '缺少 id' }, 400);
  await env.DB.prepare(`DELETE FROM company_graph WHERE record_id = ?`).bind(id).run();
  const del = await env.DB.prepare(`DELETE FROM records WHERE id = ?`).bind(id).run();
  return jsonRes({ success: true, deleted: del.meta?.changes ?? 1, id });
}
...
function normalize(str) {
  if (!str) return str;
  return str.trim().split('').map(c => T2S[c] || c).join('');
}
